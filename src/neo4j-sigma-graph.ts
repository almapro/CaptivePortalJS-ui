...
export class Neo4jSigmaGraph {
  ...
  generateSession = () =>
    this.driver ? this.driver.session(this.sessionOptions) : null;
  ...
  addNodeToGraph = (node: Node) => {
    ...
    switch (node_type) {
      ...
      case "CLIENT":
        ...
        data.label = node.properties.ip
          ? `${node.properties.ip} - ${node.properties.mac}`
          : node.properties.mac;
        data.ip = node.properties.ip;
        data.mac = node.properties.mac;
        break;
  ...
  addRelationPathToGraph = (path: Path, data: any = {}) => {
    path.segments.forEach(({ relationship, start, end }) => {
      ...
      const relationshipActualStartNodeId =
        start.identity.low === relationship.start.low
          ? startNode.id
          : endNode.id;
      const relationshipActualEndNodeId =
        start.identity.low === relationship.end.low ? startNode.id : endNode.id;
      this.addNodeToGraph(start);
      this.addNodeToGraph(end);
      if (
        !this.graph.hasEdge(
          relationshipActualStartNodeId,
          relationshipActualEndNodeId
        )
      ) {
        this.graph.addEdge(
          relationshipActualStartNodeId,
          relationshipActualEndNodeId,
          { label: relationshipType, ...data }
        );
      }
    });
  };
  ...
  getNodesByLabel = async (
    ...
    const result = await session.run(
      `MATCH (n:${_.capitalize(label.toLowerCase())}) RETURN n`
    );
    await session.close();
  ...
  getNodeById = async (
    ...
    const result = await session.run("MATCH (n { id: $nodeId }) RETURN n", {
      nodeId,
    });
    await session.close();
  ...
  getNodeRelations = async (
    ...
    if (!session) return [];
    const result = await session.run(
      "MATCH p = ({ id: $nodeId })-[r]-() WHERE NOT r:HAS_HANDSHAKE RETURN p",
      { nodeId }
    );
    const relations: Path[] = result.records.map(
      (record) => record.toObject().p
    );
    await session.close();
    return relations;
  };

  getNodesShortestPath = async (
    startNodeId: string,
    endNodeId: string,
    _session?: Session
  ): Promise<Path[]> => {
    ...
    const paths = await session.run(
      "MATCH p = shortestPath((n1 { id: $startNodeId })-[*]-(n2 { id: $endNodeId })) RETURN p",
