...
export const FloatingActions: FC<FloatingActionsProps> = ({
	...
	useEffect(() => {
		...
		if (endNode)
			setStartValues(newValues.filter((node) => node.id !== endNode));
	...
	useEffect(() => {
		...
		if (startNode)
			setEndValues(newValues.filter((node) => node.id !== startNode));
	...
	const handleStartNodeSearchChange = (searchString: string) => {
		const valueItem = startValues.find(
			(value) => value.label === searchString
		);
		if (valueItem) {
	...
	const handleEndNodeSearchChange = (searchString: string) => {
		const valueItem = endValues.find(
			(value) => value.label === searchString
		);
		if (valueItem) {
	...
	const [nodePropertiesInfo, setNodePropertiesInfo] = useState<
		NodePropertyInfoType[]
	>([]);
		...
		const asyncCallback = async () => {
			...
			const generateNodePropertiesInfoFromNode = async (node: string) => {
				...
				if (driver && graph.hasNode(node)) {
					...
					switch (node_type) {
						case "WIFI":
							newNodePropertiesInfo.push({
								nodeId,
								label: "ESSID",
								value: properties.essid,
							});
							newNodePropertiesInfo.push({
								nodeId,
								label: "BSSID",
								value: properties.bssid,
							});
								...
								newNodePropertiesInfo.push({
									...
									toggleShowPassword: () =>
										setShowPasswordFields((prev) => {
											if (_.includes(prev, properties.id))
												return prev.filter(
													(i) => i !== properties.id
												);
							...
							const wifiClientsCount = await session.run(
								"MATCH (:Wifi { id: $nodeId })<-[]-(c:Client) RETURN DISTINCT c",
								{ nodeId }
							);
							newNodePropertiesInfo.push({
								nodeId,
								label: "Clients",
								value: wifiClientsCount.records.length.toString(),
							});
							break;
						case "ROUTER":
							newNodePropertiesInfo.push({
							...
							const routerWifisCount = await session.run(
								"MATCH (:Router { id: $nodeId })-[]-(w:Wifi) RETURN DISTINCT w",
								{ nodeId }
							);
							const routerClientsCount = await session.run(
								"MATCH (:Router { id: $nodeId })-[*1..2]-(c:Client) RETURN DISTINCT c",
								{ nodeId }
							);
							newNodePropertiesInfo.push({
								nodeId,
								label: "Wifis",
								value: routerWifisCount.records.length.toString(),
							});
							newNodePropertiesInfo.push({
								nodeId,
								label: "Clients",
								value: routerClientsCount.records.length.toString(),
							});
							break;
						case "CLIENT":
							if (properties.ip)
						...
						case "WIFIPROBE":
							...
							const wifiProbeClientsCount = await session.run(
								"MATCH (:WifiProbe { id: $nodeId })-[*1]-(c:Client) RETURN DISTINCT c",
								{ nodeId }
							);
							newNodePropertiesInfo.push({
								nodeId,
								label: "Clients",
								value: wifiProbeClientsCount.records.length.toString(),
							});
							break;
						case "FLOOR":
							const floorRoutersCount = await session.run(
								"MATCH (:Floor { id: $nodeId })-[*1]-(r:Router) RETURN DISTINCT r",
								{ nodeId }
							);
							const floorWifisCount = await session.run(
								"MATCH (:Floor { id: $nodeId })-[*1..2]-(w:Wifi) RETURN DISTINCT w",
								{ nodeId }
							);
							const floorClientsCount = await session.run(
								"MATCH (:Floor { id: $nodeId })-[*2]-(c:Client) RETURN DISTINCT c",
								{ nodeId }
							);
							newNodePropertiesInfo.push({
								nodeId,
								label: "Wifis",
								value: floorWifisCount.records.length.toString(),
							});
							newNodePropertiesInfo.push({
								nodeId,
								label: "Routers",
								value: floorRoutersCount.records.length.toString(),
							});
							newNodePropertiesInfo.push({
								nodeId,
								label: "Clients",
								value: floorClientsCount.records.length.toString(),
							});
							break;
						case "BUILDING":
						case "HOUSE":
							...
							if (node_type === "BUILDING") {
								const buildingFloorsCount = await session.run(
									"MATCH (:Building { id: $nodeId })-[]-(f:Floor) RETURN DISTINCT f",
									{ nodeId }
								);
								newNodePropertiesInfo.push({
									nodeId,
									label: "Floors",
									value: buildingFloorsCount.records.length.toString(),
								});
							}
							const buildingRoutersCount = await session.run(
								"MATCH (:Building { id: $nodeId })-[*1..2]-(r:Router) RETURN DISTINCT r",
								{ nodeId }
							);
							const buildingWifisCount = await session.run(
								"MATCH (:Building { id: $nodeId })-[*1..3]-(w:Wifi) RETURN DISTINCT w",
								{ nodeId }
							);
							const buildingClientsCount = await session.run(
								"MATCH (:Building { id: $nodeId })-[*2..3]-(c:Client) RETURN DISTINCT c",
								{ nodeId }
							);
							newNodePropertiesInfo.push({
								nodeId,
								label: "Wifis",
								value: buildingWifisCount.records.length.toString(),
							});
							newNodePropertiesInfo.push({
								nodeId,
								label: "Routers",
								value: buildingRoutersCount.records.length.toString(),
							});
							newNodePropertiesInfo.push({
								nodeId,
								label: "Clients",
								value: buildingClientsCount.records.length.toString(),
							});
							break;
					}
					await session.close();
			...
			if (hoveredNode && selectedNode !== hoveredNode) {
				setNodePropertiesInfo(
					await generateNodePropertiesInfoFromNode(hoveredNode)
				);
			} else if (selectedNode) {
				setNodePropertiesInfo(
					await generateNodePropertiesInfoFromNode(selectedNode)
				);
			}
		};
		asyncCallback();
	...
	const [importFromExportedGraphFile, setImportFromExportedGraphFile] =
		useState<File | null>(null);
	const [importFromWifiteCrackedFile, setImportFromWifiteCrackedFile] =
		useState<File | null>(null);
	const [importFromAirodumpCsvFile, setImportFromAirodumpCsvFile] =
		useState<File | null>(null);
	...
	type NodeTypeString =
		...
		| "Building"
		| "Floor";
	const generateNodeQueryStringFromParams = (
		type: NodeTypeString,
		params: any
	) => {
		let paramsString = _.join(
			_.keys(params).map((key) => `${key}: $${key}`),
			", "
		);
		return `MERGE (:${type} { ${paramsString} })`;
	};
	const generateEdgeQueryStringFromRelationType = (
						...
						try {
							...
							if (!parsedGraph.nodes && !parsedGraph.edges)
								throw new Neo4jError(
									"Selected graph has no nodes nor edges!",
									"1"
								);
							const importedGraph: {
								nodes: any[];
							...
							importedGraph.nodes.forEach(async (node) => {
								...
								try {
									await trx.run(
										generateNodeQueryStringFromParams(
											nodeType,
											nodeData
										),
										nodeData
							...
							importedGraph.edges.forEach(async (edge) => {
								...
								try {
									await trx.run(
										generateEdgeQueryStringFromRelationType(
											edge_type
										),
										{
											source: edge.source,
							...
							await session.close();
							enqueueSnackbar(
								"Graph has been imported successfuly",
								{ variant: "success" }
							);
							handleCancel();
							onDoneImporting();
			...
			if (tabIndex === 1 && importFromWifiteCrackedFile) {
				reader.onload = async () => {
					if (reader.result && typeof reader.result === "string") {
						try {
							const parsedGraph = JSON.parse(reader.result);
							if (!parsedGraph.length)
								throw new Neo4jError(
									"Selected wifite cracked.json file has no nodes!",
									"1"
								);
							type WifiteNode =
								| WifiteCrackedWPA
							...
							importedGraph.forEach(async (node) => {
								...
								try {
									let queryString =
										`MERGE (n:Wifi { essid: $essid, bssid: $bssid })` +
										` ON CREATE SET n.id = $id${
											nodeData.password
												? ", n.password = $password"
												: ""
										}${
											nodeData.pin ? ", n.pin = $pin" : ""
										}`;
									if (nodeData.password || nodeData.pin)
										queryString += ` ON MATCH SET ${_.join(
											_.compact([
												nodeData.password
													? "n.password = $password"
													: "",
							...
							await session.close();
							enqueueSnackbar(
								"Wifite cracked.json has been imported successfuly",
								{ variant: "success" }
							);
							handleCancel();
							onDoneImporting();
				...
				reader.onload = async () => {
					if (reader.result && typeof reader.result === "string") {
						try {
							...
							reader.result.split("\n").forEach((row) => {
								if (!!row.trim()) parser.write(row);
							});
							parser.end();
							if (!parsedData.length)
								throw new Neo4jError(
									"Selected airodump csv file has no nodes!",
									"1"
								);
								...
								if (stationRows) {
									...
									if (clientProbes.length) {
										clientProbes.forEach((p) => {
											const wifiFound = _.find(wifis, {
												essid: p,
											});
											if (wifiFound) {
												const clientConnectsTo = {
													label: "CONNECTS_TO",
													wifi: wifiFound.bssid,
													client: row[0],
												};
												if (
													!_.find(
							...
							edges.forEach(async (edge) => {
								...
								let queryString = `MATCH (c { mac: $client }), (w { ${
									edge.label === "KNOWS"
										? "essid: $wifi"
										: "bssid: $wifi"
								} }) MERGE (c)-[:${edge.label}]->(w)`;
								await trx.run(queryString, edgeData);
							});
							await trx.commit();
							await session.close();
							enqueueSnackbar(
								"Airodump csv has been imported successfuly",
								{ variant: "success" }
							);
							handleCancel();
							onDoneImporting();
	...
	const handleExportSubmit = (e: FormEvent) => {
		...
		const dataToBeExported = {
			nodes: JSON.parse(
				JSON.stringify(
					serializedGraph.nodes.map((node) =>
						_.assign({}, node.attributes, {
							type: undefined,
							label: undefined,
							x: undefined,
							y: undefined,
			...
			edges: JSON.parse(
				JSON.stringify(
					serializedGraph.edges.map((edge) =>
						_.assign({}, edge.attributes, {
							source: edge.source,
							target: edge.target,
						})
					)
	...
	return (
		<>
			<Box
				className={classes.floatingActionsTop}
				display="grid"
				rowGap={2}
			>
				...
				<Tooltip placement="left" title="Add a node">
					<Fab color="secondary" onClick={showAddNode}>
						<AddIcon />
					</Fab>
				</Tooltip>
				<Tooltip placement="left" title="Refresh">
					<Fab color="secondary" onClick={() => sigma.refresh()}>
						<RefreshIcon />
					</Fab>
				</Tooltip>
				<Tooltip placement="left" title="Import from...">
					<Fab
						color="secondary"
			...
			<Box className={classes.floatingSearchAndFind}>
				<Grid container spacing={2} sx={{ width: 350 }}>
					<Grid
						...
						xs={9}
						flexGrow={1}
						sx={{ "& .MuiTextField-root": { my: 0.5 }, px: 0.5 }}
					>
						{!isFindPath && (
							<Autocomplete
								disablePortal
						...
						{isFindPath && (
							<>
								<Autocomplete
									...
									onInputChange={(e, v) => {
										if (e && e.type === "onChange")
											handleStartNodeSearchChange(v);
									}}
									noOptionsText={
										startNodeSearch === ""
											? "Start typing to search"
									...
									renderInput={(params) => (
										<TextField
											{...params}
											onChange={(e) =>
												handleStartNodeSearchChange(
													e.target.value
												)
											}
											label="Find a start node"
										/>
									)}
								/>
								<Autocomplete
									...
									renderInput={(params) => (
										<TextField
											{...params}
											onChange={(e) =>
												handleEndNodeSearchChange(
													e.target.value
												)
											}
											label="Find an end node"
										/>
									)}
								/>
							</>
					...
					>
						<Tooltip
							title={`${
								isFindPath ? "Disable" : "Enable"
							} find path`}
							sx={{ my: 1 }}
						>
							<IconButton
								size="large"
								color={isFindPath ? "primary" : "inherit"}
								onClick={() => setIsFindPath(!isFindPath)}
							>
								<TimelineIcon />
							</IconButton>
						</Tooltip>
			...
			<Box className={classes.floatingActionsBottomLeft}>
				<Collapse
					in={expandNodeInfo && nodePropertiesInfo.length > 0}
				...
				>
					<Paper sx={{ width: 450, p: 2 }}>
						<Grid container>
							<Grid item xs={11}>
								Node info
								{(hoveredNodeLabel || selectedNodeLabel) &&
									` (${
										hoveredNodeLabel || selectedNodeLabel
									})`}
							</Grid>
							<Grid item xs={1}>
								<Tooltip
									title={`${
										expandNodeInfo ? "Minimize" : "Expand"
									} node info`}
								>
									<IconButton
										size="small"
										onClick={() =>
											setExpandNodeInfo(!expandNodeInfo)
										}
									>
										{!expandNodeInfo ? (
											<ExpandLessIcon />
							...
							<Grid item xs={12}>
								{expandNodeInfo && (
									<>
										<List>
											{nodePropertiesInfo.map(
												(propertyInfo) => (
													<ListItem
														key={propertyInfo.label}
														secondaryAction={
															propertyInfo.secondaryAction
														}
													>
														<Grid container>
															<Grid
																			...
																			propertyInfo.pin ? (
																				<InputAdornment position="end">
																					<IconButton
																						onClick={() => {
																							if (
																								propertyInfo.toggleShowPassword
																							)
																								propertyInfo.toggleShowPassword();
																						}}
																						edge="end"
			...
			<Dialog open={showImportDialog} fullWidth maxWidth="md">
				<form onSubmit={handleImportSubmit}>
					<DialogTitle>
						...
						<Tabs
							value={tabIndex}
							onChange={(__, newValue) => setTabIndex(newValue)}
						>
					...
					<DialogContent>
						{tabIndex === 0 && (
							<Grid container spacing={2}>
								<Grid item>
									...
									>
										...
										<input
											accept=".json"
											onChange={(e) =>
												setImportFromExportedGraphFile(
								...
								<Grid item sx={{ flexGrow: 1 }}>
									{importFromExportedGraphFile && (
										<TextField
											fullWidth
											size="small"
											InputProps={{ readOnly: true }}
											value={
												importFromExportedGraphFile.path
											}
										/>
									)}
								</Grid>
						...
						{tabIndex === 1 && (
							<Grid container spacing={2}>
								<Grid item>
									...
									>
										...
										<input
											accept=".json"
											onChange={(e) =>
												setImportFromWifiteCrackedFile(
													e.target.files?.item(0) ??
														null
												)
											}
								...
								<Grid item sx={{ flexGrow: 1 }}>
									{importFromWifiteCrackedFile && (
										<TextField
											fullWidth
											size="small"
											InputProps={{ readOnly: true }}
											value={
												importFromWifiteCrackedFile.path
											}
										/>
									)}
								</Grid>
						...
						{tabIndex === 2 && (
							<Grid container spacing={2}>
								...
								<Grid item sx={{ flexGrow: 1 }}>
									{importFromAirodumpCsvFile && (
										<TextField
											fullWidth
											size="small"
											InputProps={{ readOnly: true }}
											value={
												importFromAirodumpCsvFile.path
											}
										/>
									)}
								</Grid>
			...
			<Dialog open={showExportDialog} fullWidth maxWidth="md">
				<form onSubmit={handleExportSubmit}>
					...
					<DialogContent>
						You will be exporting ({sigma.getGraph().nodes().length}
						) nodes with ({sigma.getGraph().edges().length})
						relations.
					</DialogContent>
					<DialogActions>
						<Button color="inherit" onClick={handleCancel}>
