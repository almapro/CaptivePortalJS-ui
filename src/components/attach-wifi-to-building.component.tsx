...
export const AttachWifiToBuilding: FC<AttachWifiToBuildingProps> = ({ show, close, onDone, wifiId }) => {
	...
	const attachToBuilding = async (session: Session) => {
		...
		await txr.run(`MATCH (w:Wifi { id: $wifiId }), (r { id: $rid }) CREATE (w)-[:ATTACHED_TO]->(r)`, { wifiId, rid });
		await txr.commit();
		await session.close();
	}
	useEffect(() => {
